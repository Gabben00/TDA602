...
def tokenize_instructions_qwen_chat(
    ...
    if outputs is not None:
        prompts = [
            format_instruction_qwen_chat(instruction=instruction, output=output, system=system, include_trailing_whitespace=include_trailing_whitespace, is_qwen3=is_qwen3)
            for instruction, output in zip(instructions, outputs)
        ]
    else:
        prompts = [
            format_instruction_qwen_chat(instruction=instruction, system=system, include_trailing_whitespace=include_trailing_whitespace, is_qwen3=is_qwen3)
...
def orthogonalize_qwen_weights(model, direction: Float[Tensor, "d_model"]):
    ...
    for block in model.model.layers:
        block.self_attn.o_proj.weight.data = get_orthogonalized_matrix(
            block.self_attn.o_proj.weight.data.T, direction).T
        block.mlp.down_proj.weight.data = get_orthogonalized_matrix(
            block.mlp.down_proj.weight.data.T, direction).T
...
#            attn_implementation="flash_attention_2",
    ...
    def _get_tokenize_instructions_fn(self):
        is_qwen3 = "Qwen3" in self.model.config._name_or_path
        return functools.partial(tokenize_instructions_qwen_chat, tokenizer=self.tokenizer, system=None, include_trailing_whitespace=True, is_qwen3=is_qwen3)
