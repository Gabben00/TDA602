...
def _eval_worker(cfg_artifact_path, dataset_name, label, eval_methodologies):
    # fresh process, clean CUDA context
    import json
    from pipeline.submodules.evaluate_jailbreak import evaluate_jailbreak
...
def select_and_save_direction(cfg, model_base, harmful_val, harmless_val, candidate_directions):
    ...
    try:
        ...
        with open(os.path.join(cfg.artifact_path(), "select_direction/direction_evaluations.json")) as f:
            all_scores = json.load(f)
        kl_vals = [s['kl_div_score'] for s in all_scores]
        st_vals  = [s['steering_score'] for s in all_scores]
        rs_vals  = [s['refusal_score'] for s in all_scores]
        print(f"[debug] KL div:       min={min(kl_vals):.4f}  max={max(kl_vals):.4f}  mean={sum(kl_vals)/len(kl_vals):.4f}")
        print(f"[debug] Steering:     min={min(st_vals):.4f}  max={max(st_vals):.4f}  mean={sum(st_vals)/len(st_vals):.4f}")
        print(f"[debug] Refusal:      min={min(rs_vals):.4f}  max={max(rs_vals):.4f}  mean={sum(rs_vals)/len(rs_vals):.4f}")
        raise
...
def run_pipeline(args):
    ...
    print(f"[debug] refusal_toks: {model_base.refusal_toks}")
    print(f"[debug] refusal_toks decoded: {model_base.tokenizer.batch_decode(model_base.refusal_toks)}")
    test_prompt = harmful_val[0]
    toks = model_base.tokenize_instructions_fn(instructions=[test_prompt])
    with torch.no_grad():
        logits = model_base.model(input_ids=toks.input_ids.to(model_base.model.device), attention_mask=toks.attention_mask.to(model_base.model.device)).logits
    top_toks = logits[0, -1].topk(20)
    print(f"[debug] top 20 next tokens: {model_base.tokenizer.batch_decode(top_toks.indices)}")
    print(f"[debug] top 20 token ids: {top_toks.indices.tolist()}")
    ...
    think_end = model_base.tokenizer.encode("</think>", add_special_tokens=False)
    print(f"[debug] </think> token ids: {think_end}")
    print(f"[debug] </think> decoded: {model_base.tokenizer.batch_decode(think_end)}")

    # Generate a short completion to see what refusal looks like
    with torch.no_grad():
        out = model_base.model.generate(
            toks.input_ids.to(model_base.model.device),
            max_new_tokens=50,
            do_sample=False
        )
    print(f"[debug] generation: {model_base.tokenizer.decode(out[0])}")

    # Find what token follows </think> in the generation
    generated = out[0].tolist()
    for i, t in enumerate(generated):
        if t == think_end[-1]:
            print(f"[debug] token after </think>: id={generated[i+1]}, decoded='{model_base.tokenizer.decode([generated[i+1]])}'")
            break
